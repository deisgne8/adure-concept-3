...
function updateHeader() {
  header.classList.toggle(
    "is-glass",
    scrollY > Math.max(24, hero.offsetHeight * 0.08),
  );
}
updateHeader();
...
function render(items) {
  ...
  $("#home-properties").innerHTML = items.length
    ? items
        .map((p) =>
          [
            ...
            `<a class="property-image" href="${reference}#property-detail" aria-label="View ${p.title}">`,
            `<img src="assets/${p.image}" alt="${p.title}" width="640" height="400" loading="lazy">`,
            `<span class="tag">Available</span>`,
            `</a>`,
            `<div class="property-copy">`,
            `<h3>${p.title}</h3>`,
            `<p class="meta">${p.place}</p>`,
            `<p class="price">${format(p)}</p>`,
            `<p class="meta">${p.facts}</p>`,
            `<p class="meta">Assigned contact · ${p.contact}</p>`,
            `<div class="property-actions">`,
            `<a class="btn link" href="${reference}#property-detail">View property</a>`,
            `<div class="property-utilities">`,
            `<button class="utility-button" data-favourite="${p.id}" aria-label="Save ${p.title}" aria-pressed="${favourites.includes(p.id)}">`,
    ...
    : [
        ...
        "No properties match these filters. Try another selection or ",
        '<button class="text-link" id="empty-reset">clear filters</button>.',
        "</p>",
      ].join("");
}
...
function search() {
  ...
  $("#search-status").textContent =
    `${results.length} ${results.length === 1 ? "property" : "properties"} available to ${intent === "buy" ? "buy" : "lease"}.`;
}
...
function setMoreFiltersOpen(open) {
  ...
  if (iconPath)
    iconPath.setAttribute(
      "d",
      open ? "M5 5l10 10M15 5L5 15" : "M3 5h14M3 10h14M3 15h14M7 3v4m6 1v4",
    );
  updateAmenitySummary();
  if (open) {
    moreFiltersPanel.hidden = false;
    requestAnimationFrame(() =>
      requestAnimationFrame(() => moreFiltersPanel.classList.add("is-visible")),
    );
  } else {
    moreFiltersPanel.classList.remove("is-visible");
    const finish = () => {
      if (moreFiltersToggle.getAttribute("aria-expanded") === "false")
        moreFiltersPanel.hidden = true;
    };
    if (matchMedia("(prefers-reduced-motion: reduce)").matches) finish();
    else moreFiltersTimer = setTimeout(finish, 320);
  }
}
function updateAmenitySummary() {
  const count = form.querySelectorAll('input[name="amenity"]:checked').length;
  const action =
    moreFiltersToggle.getAttribute("aria-expanded") === "true"
...
function reset() {
  ...
  render(properties.slice(0, 3));
  $("#search-status").textContent =
    "Filters cleared. Showing featured properties.";
}
...
services.addEventListener("click", () => {
  ...
  services.setAttribute("aria-expanded", String(!servicesMenu.hidden));
});
document.addEventListener("click", async (e) => {
  if (!e.target.closest(".nav-disclosure")) closeServices();
  if (e.target.closest("#empty-reset")) reset();
  const save = e.target.closest("[data-favourite]");
  if (save) {
    ...
    favourites = favourites.includes(id)
      ...
      : [...favourites, id];
    try {
      localStorage.setItem("adure-favourites", JSON.stringify(favourites));
    } catch {}
    save.setAttribute("aria-pressed", String(favourites.includes(id)));
    save.textContent = favourites.includes(id) ? "♥" : "♡";
    $("#action-status").textContent = favourites.includes(id)
      ? "Property saved."
      : "Property removed from saved items.";
  }
  const share = e.target.closest("[data-share]");
  if (share) {
    const p = properties.find((x) => x.id === share.dataset.share);
    const text = `${p.title} — ${p.place} — ${format(p)}\n${reference}#property-detail`;
    try {
      await navigator.clipboard.writeText(text);
      share.textContent = "Copied";
      $("#action-status").textContent = "Property details copied to clipboard.";
    } catch {
