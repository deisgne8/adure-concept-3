...
if (propertySearchForm) {
  ...
  const filterSlot = $("#filter-slot");
  const filterClose = filterDialog?.querySelector(".filter-close");
  ...
  const propertyCards = $("#home-properties");
  const propertySliderQuery = window.matchMedia("(max-width: 1023.98px)");
  let propertySlider;

  const destroyPropertySlider = () => {
    propertySlider?.destroy(true, true);
    propertySlider = undefined;
  };
  const syncPropertySlider = () => {
    destroyPropertySlider();
    const slider = propertyCards.querySelector(".home-properties-swiper");
    const slides = slider?.querySelectorAll(".swiper-slide") ?? [];
    if (!propertySliderQuery.matches || slides.length < 2) return;

    propertySlider = new Swiper(slider, {
  ...
  const render = (items) => {
    ...
    propertyCards.innerHTML = items.length
      ? items
          .map((property, index) =>
            [
              index === 0 ? '<div class="home-properties-swiper swiper"><div class="swiper-wrapper">' : "",
              `<div class="aos-card-reveal swiper-slide" data-aos="fade-up" data-aos-delay="${Math.min(index, 4) * 100}">`,
    ...
    requestAnimationFrame(syncPropertySlider);
  };
  const search = () => {
  ...
  render(properties.slice(0, 3));
  propertySliderQuery.addEventListener("change", syncPropertySlider);
  propertyCards.addEventListener("home-properties-list-visible", () => {
    propertySlider?.update();
    propertySlider?.autoplay.start();
  });
