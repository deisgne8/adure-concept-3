...
const philosophyItems = [
  {
    title: "Positioning",
    image: "architecture-curves",
    alt: "Curved contemporary waterfront residences",
  },
  {
    title: "Performance",
    image: "architecture-facade",
    alt: "Ordered balconies and windows across a contemporary residential facade",
  },
  {
    title: "Stewardship",
    image: "architecture-waterfront",
    alt: "Waterfront residences in warm evening light",
  },
];
...
function pausePhilosophy() {
  philosophyProgress?.pause();
}
function resumePhilosophy() {
  if (
    philosophyVisible &&
    !reduced.matches &&
    document.visibilityState === "visible"
  )
    philosophyProgress?.play();
}
function runPhilosophyProgress() {
  philosophyProgress?.cancel();
  philosophyProgress = null;
  ...
  philosophyProgress = fill.animate(
    [{ transform: "scaleX(0)" }, { transform: "scaleX(1)" }],
    { duration: philosophyDuration, easing: "linear", fill: "forwards" },
  );
  philosophyProgress.onfinish = () => {
    if (philosophyVisible)
      selectPhilosophy((philosophyIndex + 1) % philosophyItems.length, false);
  };
  if (!philosophyVisible || document.visibilityState !== "visible")
    philosophyProgress.pause();
}
function selectPhilosophy(index, announce = true) {
  if (index < 0 || index >= philosophyItems.length) return;
  philosophyIndex = index;
  const data = philosophyItems[index];
  philosophyTabs.forEach((tab, i) => {
    const selected = i === index;
    tab.setAttribute("aria-selected", String(selected));
    tab.tabIndex = selected ? 0 : -1;
  });
  const panel = $("#philosophy-panel");
  panel.setAttribute("aria-labelledby", "philosophy-tab-" + index);
  changePicture($("#philosophy-image"), data.image, data.alt);
  $("#philosophy-current").textContent = "0" + (index + 1);
  if (announce)
    $("#philosophy-announcement").textContent =
      data.title + ". " + data.alt + ".";
  runPhilosophyProgress();
}
philosophyTabs.forEach((tab) =>
  on(tab, "click", () => selectPhilosophy(Number(tab.dataset.valueTab))),
);
const philosophyObserver = new IntersectionObserver(
  (entries) => {
    philosophyVisible = entries[0].isIntersecting;
    if (philosophyVisible) {
      if (!philosophyProgress) runPhilosophyProgress();
      else resumePhilosophy();
...
if (!reduced.matches && "IntersectionObserver" in window) {
  revealTargets.forEach((element, index) => {
    element.dataset.reveal = element.matches(
      ".journey-grid,.management-layout,.portfolio-feature,.closing-content",
    )
      ? "masked"
      : "line";
...
async function changePicture(element, name, alt, animated = true) {
  const epoch = (imageRequests.get(element) || 0) + 1;
  imageRequests.set(element, epoch);
  ...
  try {
    ...
    /* Keep the supplied alt text if loading fails. */
  }
...
function paintHeader() {
  ...
  header.classList.toggle(
    "is-sticky",
    $("#perspective").getBoundingClientRect().top <= trigger,
  );
}
...
on(window, "resize", () => {
  if (innerWidth > 800 && !menu.hidden) closeMenu();
});
...
$$("[data-journey-card]").forEach((link) =>
  on(link, "click", (e) => {
    const journey = link.dataset.journeyCard,
      data = journeys[journey];
    if (data)
      $("#journey-announcement").textContent =
        journey.charAt(0).toUpperCase() + journey.slice(1) + ". " + data.body;
    if (journey === "buy" || journey === "rent") {
      setIntent(journey);
      runSearch();
    } else if (journey === "sell") {
      e.preventDefault();
      openEnquiry("sell");
    }
  }),
);
$$("[data-nav-intent]").forEach((link) =>
  on(link, "click", () => {
    setIntent(link.dataset.navIntent);
    runSearch();
  }),
);
...
const budgetOptions = {
  ...
  rent: [
    ["any", "Any annual rent"],
    ["under-100k", "Under AED 100K"],
    ["100k-200k", "AED 100K–200K"],
    ["200k-plus", "AED 200K+"],
  ],
};
function setIntent(intent) {
  ...
  budget.innerHTML = budgetOptions[intent]
    .map(
      ([value, label]) =>
        '<option value="' + value + '">' + label + "</option>",
    )
    .join("");
  if (budgetOptions[intent].some(([value]) => value === current))
...
function renderCollections(items, intent) {
  results.innerHTML = items
    .map(
      (item) =>
        '<article class="collection-card"><button type="button" class="collection-image" data-collection="' +
        item.id +
        ...
        '.webp" srcset="assets/' +
        item.image +
        "-small.webp 600w, assets/" +
        item.image +
        '.webp 900w" sizes="(max-width:600px) 85vw, 36vw" alt="' +
        escape(item.alt) +
        '" width="900" height="488" loading="lazy"></button><div class="collection-body"><p class="collection-meta">' +
        escape(item.location) +
        " · " +
        escape(item.type) +
        "</p><h3>" +
        escape(item.name) +
        '</h3><div class="collection-bottom"><button type="button" data-collection="' +
        item.id +
        '">Enquire now <span aria-hidden="true">↗</span></button></div></div></article>',
    )
    .join("");
...
async function runSearch() {
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, reduced.matches ? 0 : 180),
  );
...
on(searchForm, "change", (e) => {
  if (e.target.name === "intent") {
    ...
    $("#search-status").textContent = "Filters updated. Search to see matches.";
});
...
on($("#reset-search"), "click", resetSearch);
on($("#empty-reset"), "click", resetSearch);
renderCollections(collections, "buy");

// Three management pillars with one coordinated visual.
$("#management-index").innerHTML = management
  .map(
    (item, i) =>
      '<article class="management-entry"><h3><button type="button" id="management-button-' +
      i +
      '" data-management="' +
      i +
      '" aria-expanded="' +
      (i === 0) +
      '" aria-controls="management-detail-' +
      i +
      '"><span>' +
      escape(item.title) +
      '</span><span aria-hidden="true">' +
      (i === 0 ? "−" : "+") +
      '</span></button></h3><div class="management-detail" id="management-detail-' +
      i +
      '" role="region" aria-labelledby="management-button-' +
      i +
      '"' +
      (i === 0 ? "" : " hidden") +
      "><p>" +
      escape(item.body) +
      "</p><ul>" +
      item.examples.map((label) => "<li>" + escape(label) + "</li>").join("") +
      "</ul></div></article>",
  )
  .join("");
on($("#management-index"), "click", (e) => {
  const button = e.target.closest("[data-management]");
  if (!button) return;
  const index = Number(button.dataset.management),
    data = management[index];
  $$("[data-management]").forEach((b, i) => {
    b.setAttribute("aria-expanded", String(i === index));
    b.lastElementChild.textContent = i === index ? "−" : "+";
    $("#management-detail-" + i).hidden = i !== index;
  });
  changePicture($("#management-image"), data.image, data.alt, false);
  $("#management-outcome").textContent = data.outcome;
});
$$("[data-proof]").forEach(
  (el) => (el.textContent = site.proof[el.dataset.proof]),
);

function showGallery(index) {
  galleryIndex = Math.max(0, Math.min(portfolio.length - 1, index));
  const item = portfolio[galleryIndex],
    img = $("#gallery-image");
  img.src = "assets/" + item.image + ".webp";
  img.alt = item.alt;
  $("#gallery-caption").textContent = item.caption;
  $("#gallery-count").textContent = "0" + (galleryIndex + 1) + " / 05";
  $("#gallery-prev").disabled = galleryIndex === 0;
  $("#gallery-next").disabled = galleryIndex === portfolio.length - 1;
}
on($("#gallery-prev"), "click", () => showGallery(galleryIndex - 1));
on($("#gallery-next"), "click", () => showGallery(galleryIndex + 1));
on($("#gallery-dialog"), "keydown", (e) => {
...
function selectStage(index) {
  ...
  stageIndex = index;
  const data = transition[index];
  $$("[data-stage]").forEach((button, i) => {
    button.setAttribute("aria-selected", String(i === index));
    button.tabIndex = i === index ? 0 : -1;
  });
  $("#stage-panel").setAttribute("aria-labelledby", "stage-tab-" + index);
  $("#stage-week").textContent = data.week + " / 30-day transition";
  $("#stage-number").textContent = "0" + (index + 1);
  $("#stage-count").textContent = "0" + (index + 1) + " / 04";
  $("#stage-title").textContent = data.title + ".";
  $("#stage-body").textContent = data.body;
  $("#stage-gain").textContent = data.gain;
  $("#stage-evidence").innerHTML = data.evidence
    .map((item) => "<li>" + escape(item) + "</li>")
    .join("");
  $("#next-stage").innerHTML =
    (data.next ? "Next: " + escape(data.next) : "Discuss your transition") +
    ' <span aria-hidden="true">' +
    (data.next ? "→" : "↗") +
    "</span>";
  $("#stage-panel").style.setProperty("--stage-progress", (index + 1) / 4);
  animateChange($("#stage-title"), 250);
}
$$("[data-stage]").forEach((button) =>
  on(button, "click", () => selectStage(Number(button.dataset.stage))),
);
on($("#next-stage"), "click", () => {
  if (stageIndex < 3) selectStage(stageIndex + 1);
  else openEnquiry("manage");
});
...
if (approvedClients.length) {
  $("#client-logos").hidden = false;
  $("#client-logos").innerHTML = approvedClients
    .map(
      (c) =>
        '<img src="' +
        escape(c.image) +
...
const enquiryNames = {
  buy: "A place for your ambitions.",
  sell: "Your property. New possibilities.",
  rent: "A place to feel at home.",
  manage: "A wider view of your property.",
  general: "Let’s see what’s possible.",
};
function openEnquiry(intent = "general", collection = null) {
  if (!enquiryNames[intent]) intent = "general";
  closeMenu();
  $("#enquiry-interest").value = intent;
  $("#enquiry-heading").textContent = enquiryNames[intent];
  $("#enquiry-draft").hidden = true;
  enquiryForm.hidden = false;
  $("#draft-status").textContent = "";
  if (collection)
    enquiryForm.elements.message.value =
      "I would like to discuss " +
      (intent === "rent" ? "leasing" : "buying") +
...
on(
  $("#enquiry-interest"),
  "change",
  (e) => ($("#enquiry-heading").textContent = enquiryNames[e.target.value]),
);
on(document, "click", (e) => {
  const enquiryButton = e.target.closest("[data-enquiry]");
  if (enquiryButton) {
    openEnquiry(enquiryButton.dataset.enquiry);
    return;
  }
  const collectionButton = e.target.closest("[data-collection]");
  if (collectionButton) {
    const item = collections.find(
      (c) => c.id === collectionButton.dataset.collection,
    );
    if (item) openEnquiry(currentIntent(), item);
    return;
  }
  const galleryButton = e.target.closest("[data-portfolio]");
  if (galleryButton) {
    showGallery(Number(galleryButton.dataset.portfolio));
...
on(enquiryForm, "submit", (e) => {
  ...
  const body = [
    ...
    "Location: " + (values.location || "Not decided yet"),
    "",
    "Name: " + values.name,
    "Email: " + values.email,
    "",
  ...
  $("#enquiry-draft").hidden = false;
  $("#draft-preview").focus();
});
on($("#edit-draft"), "click", () => {
  $("#enquiry-draft").hidden = true;
  enquiryForm.hidden = false;
  enquiryForm.elements.message.focus();
});
on($("#copy-draft"), "click", async () => {
  try {
    await navigator.clipboard.writeText($("#draft-preview").value);
    $("#draft-status").textContent =
      "Draft copied. You can paste it into your email app.";
  } catch {
    $("#draft-preview").focus();
    $("#draft-preview").select();
    $("#draft-status").textContent =
...
function paintHero() {
  ...
  const raw = reduced.matches
    ? 0
    : Math.max(0, Math.min(1, -rect.top / runway));
  const p = 1 - Math.pow(1 - raw, 1.15);
  const travel =
    Math.min(innerWidth * (innerWidth <= 600 ? 1.05 : 1.12), 1560) * p;
  const parallax = -Math.min(innerHeight * 0.045, 42) * p;
  const scale = 1.035 + 0.025 * p;
...
function queueHero() {
  if (!frame && heroVisible) frame = requestAnimationFrame(paintHero);
}
const observer = new IntersectionObserver((entries) => {
  heroVisible = entries[0].isIntersecting;
  if (heroVisible) queueHero();
});
observer.observe($("#home"));
on(window, "scroll", queueHero, { passive: true });
on(window, "resize", queueHero);
on(reduced, "change", () => {
  paintHero();
});
