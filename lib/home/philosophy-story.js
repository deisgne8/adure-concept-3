...
if (stage) {
  ...
  const imageObserver = new IntersectionObserver(
    (entries) => {
      ...
      pairs.forEach((pair) =>
        pair.querySelectorAll("img").forEach((image) => {
          image.loading = "eager";
        }),
      );
  ...
  function range() {
    ...
    return {
      ...
      distance: Math.max(1, stage.offsetHeight - sticky.offsetHeight),
    };
  }
  function update() {
    frame = 0;
    ...
    select(
      Math.min(
        steps.length - 1,
        Math.max(0, Math.floor(((scrollY - start) / distance) * steps.length)),
      ),
    );
  }
  function queue() {
