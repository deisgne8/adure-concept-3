...
const ensureLeaflet = () => loadScript("/vendor/leaflet.js");
...
switcher.innerHTML = [
  '<button type="button" data-property-view="list" aria-pressed="false" aria-controls="home-properties">',
  '<span aria-hidden="true">☷</span> List view',
  "</button>",
  '<button type="button" data-property-view="map" aria-pressed="true" aria-controls="property-map-view">',
...
panel.innerHTML = [
  '<div id="property-map-canvas" aria-label="Property locations map. Use arrow keys to pan and plus or minus to zoom."></div>',
  '<aside class="property-map-locations" aria-label="Available locations">',
  '<div class="map-list-heading">Our locations <span class="map-result-count"></span></div>',
  '<div class="map-location-list"></div>',
  '<small class="map-location-note">Approximate locations shown</small>',
  "</aside>",
  '<article class="map-property-preview" aria-label="Selected property" hidden></article>',
  '<div class="map-load-message" role="status" hidden>Map imagery is unavailable. You can still explore the properties using the location list.</div>',
  '<button class="map-fit" type="button" aria-label="Show all property locations">⌖ <span>Show all</span></button>',
].join("");
cards.before(panel);
cards.hidden = true;
...
const small = () => matchMedia("(max-width: 1023.98px)").matches;

async function loadEnglishBasemap() {
  const message = panel.querySelector(".map-load-message");
  try {
    await loadScript("/vendor/maplibre-gl-5.6.2.js");
    await loadScript("/vendor/leaflet-maplibre-gl-0.1.3.js");
...
function fit() {
  ...
  map.fitBounds(items.map((p) => propertyLocations[p.id]), {
    ...
    paddingBottomRight: desktop ? [preview.offsetWidth + 65, 70] : [35, 65],
    maxZoom: 12,
    animate: true,
  });
}
...
function select(id, move, showPreview = previewVisible) {
  const p = items.find((property) => property.id === id) || items[0];
  if (!p) { previewVisible = false; preview.hidden = true; return; }
  selectedId = p.id;
  previewVisible = Boolean(showPreview);
  ...
  if (previewVisible) preview.innerHTML = [
    `<img src="assets/${escape(p.image)}" alt="${escape(p.title)}" width="640" height="400">`,
    '<div class="map-preview-copy">', `<span class="map-preview-place">${escape(p.place)}</span>`, `<h3>${escape(p.title)}</h3>`,
    `<div class="map-preview-price">${price(p)}</div>`, `<div class="map-preview-facts">${escape(p.facts)}</div>`,
    `<a class="btn primary" href="${reference}#property-detail">View property${buttonArrow}</a>`, "</div>",
  ].join("");
  if (move && map) map.flyTo(propertyLocations[p.id], 12, { animate: true, duration: 0.8 });
}
...
export function updatePropertyMap(results) {
  ...
  list.innerHTML = items.length ? items.map((p) => [
    `<button type="button" class="map-location" data-map-property="${p.id}" aria-pressed="false">`,
    '<span class="map-location-media" aria-hidden="true">', `<img src="assets/${escape(p.image)}" alt="" width="360" height="270" loading="lazy" decoding="async">`, "</span>",
    '<span class="map-location-details">', `<strong>${escape(p.title)}</strong>`, `<span class="map-location-address">${escape(p.place)}</span>`,
    `<span class="map-location-facts">${escape(p.facts)}</span>`, '<span class="map-location-footer">',
    `<span class="map-location-action">View on map ${buttonArrow}</span>`, `<span class="map-location-price">${price(p)}</span>`, "</span>", "</span>", "</button>",
  ].join("")).join("") : '<div class="map-empty">No properties match these filters. Adjust your search or <button class="text-link" type="button" id="map-empty-reset">clear filters</button>.</div>';
...
switcher.addEventListener("click", (event) => {
  ...
  panel.hidden = view !== "map";
  cards.hidden = view !== "list";
  if (view === "map") { initMap(); requestAnimationFrame(fit); }
  else cards.dispatchEvent(new CustomEvent("home-properties-list-visible"));
});
const observer = new IntersectionObserver((entries) => {
  if (entries.some((entry) => entry.isIntersecting)) { initMap(); observer.disconnect(); }
}, { rootMargin: "300px" });
