...
(() => {
  ...
  function resetVideo() {
    animations
      .filter((animation) => animation.effect?.target === video)
  ...
  listen(
    ...
    () => {
      ...
      if (root.dataset.intro && scrollY > 0)
        scrollTo({ top: 0, behavior: "instant" });
  ...
  async function play() {
    ...
    await Promise.race([
      Promise.all([
        document.fonts.load('400 64px "Fira Sans"'),
        poster.decode(),
      ]).catch(() => {}),
      pause(1000),
    ]);
    ...
    if (lockup.scrollWidth > lockup.clientWidth)
      intro.classList.add("intro-stacked");
    const slot = intro
      .querySelector(".intro-image-slot")
    ...
    const targetScale =
      Number(heroStyle.transform.match(/^matrix\(([^,]+)/)?.[1]) || 1;
    const smallScale = Math.max(
    ...
    const phraseGap = stacked
      ? afterRect.top - beforeRect.bottom
      : afterRect.left - beforeRect.right;
    const textGap =
      parseFloat(getComputedStyle(before).fontSize) * (stacked ? 0.14 : 0.24);
    const closeGap = Math.max(0, (phraseGap - textGap) / 2);
    const compactBefore = stacked
      ? `translate(0px, ${closeGap}px)`
      : `translate(${closeGap}px, 0px)`;
    const compactAfter = stacked
      ? `translate(0px, ${-closeGap}px)`
      : `translate(${-closeGap}px, 0px)`;
    const closed = {
    ...
    const move = (horizontal, vertical) =>
      `translate3d(${horizontal}px, ${vertical}px, 0px)`;
    ...
    maskTransforms(closed, 1).forEach((transform, index) => {
      masks[index].style.transform = transform;
    });
    ...
    const exitDistance = (stacked ? viewport.height : viewport.width) * 0.07;
    for (let index = 0; index <= 66; index++) {
      ...
      const expansion =
        growth < 0.9 ? growth / 0.95 : 1 - (1 - growth) ** 2 / 0.19;
      const box = {
        left: lerp(lerp(closed.left, slot.left, opening), 0, expansion),
        right: lerp(
          lerp(closed.right, slot.right, opening),
          viewport.width,
          expansion,
        ),
        top: lerp(lerp(closed.top, slot.top, opening), 0, expansion),
        bottom: lerp(
          lerp(closed.bottom, slot.bottom, opening),
          viewport.height,
          expansion,
        ),
      };
      maskTransforms(box, 1 - expansion).forEach((transform, side) => {
        maskFrames[side].push({ offset, transform });
      });
      videoFrames.push({
        offset,
        transform: `${move(lerp(x, fullX, expansion), lerp(y, fullY, expansion))} scale(${lerp(smallScale, targetScale, expansion)})`,
      });
      ...
      beforeFrames.push({
        offset,
        opacity,
        transform: stacked ? move(0, shift) : move(shift, 0),
      });
      afterFrames.push({
        offset,
        opacity,
    ...
    const arrival = [
      ...
      animate(
        hero.querySelector(".hero-shade"),
        [{ opacity: 0 }, { opacity: 1 }],
        { duration: 450 },
      ),
      animate(document.querySelector(".site-header"), reveal, {
        duration: 650,
        delay: 220,
        easing: "ease-out",
      }),
      animate(hero.querySelector("h1"), reveal, {
        duration: 700,
        delay: 130,
        easing: "ease-out",
      }),
      animate(hero.querySelector(".hero-kicker"), reveal, {
        duration: 550,
        delay: 330,
        easing: "ease-out",
      }),
      animate(hero.querySelector(".hero-bottom"), reveal, {
        duration: 650,
        delay: 440,
        easing: "ease-out",
      }),
      animate(hero.querySelector(".hero-foot"), reveal, {
        duration: 550,
        delay: 560,
        easing: "ease-out",
      }),
