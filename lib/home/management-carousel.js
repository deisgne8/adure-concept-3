...
if (root) {
  ...
  const serviceMedia = [
    [
      "assets/hidd-al-saadiyat/management-leasing-lobby.jpg",
      "Hidd Al Saadiyat lobby with a sculptural chandelier and timber screen",
    ],
    [
      "assets/hidd-al-saadiyat/management-facility-facade.jpg",
      "Hidd Al Saadiyat mixed-use facade and contemporary residences",
    ],
    [
      "assets/hidd-al-saadiyat/urban-mixed-use.webp",
      "Hidd Al Saadiyat mixed-use property and active street frontage",
    ],
  ];
  ...
  if (
    ...
    image.alt =
      "Beachfront residences, palm-lined promenade and turquoise sea at Hidd Al Saadiyat";
    ...
    function entrance(progress) {
      ...
      root.style.setProperty(
        "--management-scene-width",
        `${68 + expansion * 32}%`,
      );
      root.style.setProperty(
        "--management-scene-height",
        `${74 + expansion * 26}%`,
      );
      root.style.setProperty(
        "--management-scene-y",
        `${((1 - rise) * stageHeight * 0.03).toFixed(2)}px`,
      );
      root.style.setProperty(
        "--management-scene-radius",
        `${(1 - expansion) * 30}px`,
      );
      root.style.setProperty(
        "--management-scene-border",
        `${(1 - expansion) * 8}px`,
      );
      const showCopy = progress >= 0.9999;
    ...
    function renderEntrance(now) {
      const elapsed = entranceTime
        ? Math.min(64, now - entranceTime)
        : 1000 / 60;
      entranceTime = now;
      // Match the reference's 0.1 interpolation at 60 Hz at any refresh rate.
      entrancePosition +=
        (entranceTarget - entrancePosition) *
        (1 - Math.pow(0.9, elapsed / (1000 / 60)));
      if (Math.abs(entranceTarget - entrancePosition) < 0.0001)
        entrancePosition = entranceTarget;
      entrance(entrancePosition);
      if (entrancePosition !== entranceTarget)
    ...
    function measure() {
      ...
      root.style.setProperty(
        ...
        `${stageHeight * 0.84 + entranceDistance}px`,
      );
      root.style.setProperty(
        "--management-title-height",
        `${title.getBoundingClientRect().height}px`,
      );
      root.style.setProperty(
        "--management-intro-height",
        `${intro.getBoundingClientRect().height}px`,
      );
      sync();
    }
    ...
    function sync() {
      ...
      const introProgress = reduced
        ? 1
        : compact.matches
          ? clamp((innerHeight - trackRect.top) / (stageHeight * 0.95))
      ...
      const progress = Math.max(
        0,
        Math.min(1, (headerHeight - trackRect.top) / travel),
      );
      root.style.setProperty(
        "--management-image-y",
        `${Math.round((1 - progress) * stageHeight * 0.045)}px`,
      );

      let active = 0;
      const cardTop =
        headerHeight + Math.max(32, Math.min(stageHeight * 0.07, 72));
      slides.forEach((slide, index) => {
      ...
      slides.forEach((slide, index) => {
        ...
        const entering = Math.max(
          ...
          Math.min(
            1,
            (stageHeight - rect.top) / Math.max(1, stageHeight - cardTop),
          ),
        );
        ...
        const stacked = next
          ? Math.max(
              ...
              Math.min(
                1,
                (stageHeight - nextTop) / Math.max(1, stageHeight - cardTop),
              ),
            )
          : 0;
        slide.style.setProperty(
          "--service-scale",
          (1 - stacked * 0.1).toFixed(4),
        );
        slide.style.setProperty(
          "--service-overlay-opacity",
          (stacked * 0.18).toFixed(4),
        );
        slide.style.setProperty(
          "--service-image-scale",
          (1.3 - entering * 0.3).toFixed(4),
        );
