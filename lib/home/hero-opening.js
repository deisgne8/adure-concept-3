const root = document.documentElement,
  hero = document.querySelector(".site-hero");
...
const animations = [],
  background = [
    ...document.querySelectorAll(".site-header,main,.footer,.skip-link"),
  ];
const previous = background.map((element) => element.inert);
const OPENING_DURATION_SCALE = 1;
const wait = (ms) =>
  new Promise((resolve) => setTimeout(resolve, ms * OPENING_DURATION_SCALE));
const clamp = (min, value, max) => Math.max(min, Math.min(max, value));
const animate = (element, frames, options) => {
  const timing = {
    ...options,
    duration: options.duration * OPENING_DURATION_SCALE,
  };
  if (options.delay != null)
    timing.delay = options.delay * OPENING_DURATION_SCALE;
...
function criticalAssetsReady() {
  ...
  if (logo && !(logo.complete && logo.naturalWidth))
    assets.push(Promise.race([once(logo, "load"), once(logo, "error")]));
  if (video && video.readyState < 2)
    assets.push(
      Promise.race([once(video, "loadeddata"), once(video, "error")]),
    );
...
function finish() {
  ...
  const completedHandoff = root.dataset.opening === "handoff";

  if (completedHandoff) {
    const heroStyles = getComputedStyle(hero);
    const mediaScale =
      heroStyles.getPropertyValue("--hero-media-scale").trim() || "1";
    const mediaY =
      heroStyles.getPropertyValue("--hero-media-y").trim() || "0px";

    // Match the component's resting transform before removing intro-only styles.
    video.style.transform = `translate3d(0, ${mediaY}, 0) scale(${mediaScale})`;
    video.style.clipPath = "inset(0 0 0 0)";
    requestAnimationFrame(() => video.removeAttribute("style"));
  } else {
    video.removeAttribute("style");
  }
  intro.hidden = true;
  delete root.dataset.opening;
  root.dataset.introAnimationComplete = "true";
  document.dispatchEvent(new Event("adure:intro-complete"));
  ...
  if (
    ...
    const target = document.getElementById(
      decodeURIComponent(landingHash.slice(1)),
    );
    if (target)
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && !done) finish();
});
intro?.querySelector(".intro-skip")?.addEventListener("click", finish);
window.addEventListener("resize", () => {
  // Embedded browsers can emit a startup resize without changing dimensions.
  if (
    !done &&
    performance.now() - openingStartedAt > 500 &&
    measuredViewport &&
    (Math.abs(innerWidth - measuredViewport.width) > 2 ||
      Math.abs(innerHeight - measuredViewport.height) > 2)
  )
    finish();
});
...
async function open() {
  ...
  const handoffDuration = 560;
  video.style.cssText =
    "inset:0;width:100%;height:100%;opacity:1;object-fit:cover;transform-origin:50% 50%;clip-path:inset(100% 0 0 0);border:0;border-radius:0";
  ...
  rings.forEach((ring, index) =>
    animate(
      ...
      [
        {
          opacity: 0,
          transform: `translate(-50%,-50%) rotate(${index ? -150 : 150}deg) scale(.78)`,
        },
        {
          opacity: 0.72,
    ...
    [
      {
        ...
        transform: "translateX(38%) scale(.94)",
      },
      {
        opacity: 1,
        clipPath: "inset(0 76% 0 0)",
  ...
  const targetLeft =
    navLogoBox?.left ||
    (innerWidth <= 767 ? 20 : clamp(30, innerWidth * 0.045, 64));
  ...
  animate(
    ...
    [
      ...
      {
        opacity: 1,
        transform: `translate3d(${targetLeft - brandBox.left}px,${targetTop - brandBox.top}px,0) scale(${brandScale})`,
      },
    ],
    { duration: revealDuration, easing: "cubic-bezier(.22,.61,.36,1)" },
  );
  animate(
  ...
  rings.forEach((ring) =>
    animate(ring, [{ opacity: 0.22 }, { opacity: 0 }], {
      duration: 560,
      easing: "cubic-bezier(.22,.61,.36,1)",
    }),
  );
  const heroScale =
    getComputedStyle(hero).getPropertyValue("--hero-media-scale").trim() || "1";
  const revealStartScale = (Number.parseFloat(heroScale) || 1) + 0.08;
  // Reveal the full-size playing film in one continuous movement, without a
  // contained-card hold or a second expansion phase.
  const videoReveal = animate(
    video,
    [
      {
        transform: `scale(${revealStartScale})`,
        clipPath: "inset(100% 0 0 0)",
      },
  ...
  await Promise.all([
    videoReveal,
    animate(
      intro.querySelector(".intro-lockup"),
      [{ opacity: 1 }, { opacity: 0 }],
      { duration: handoffDuration, easing: "cubic-bezier(.22,.61,.36,1)" },
    ),
  ]);
  if (done) return;
  finish();
