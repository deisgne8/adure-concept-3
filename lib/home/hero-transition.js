...
if (hero && nextSection && !reducedMotion.matches) {
  ...
  const paint = () => {
    ...
    const mediaTravel = Math.min(window.innerHeight * 0.13, 120) * eased;
    const copyTravel = Math.min(window.innerHeight * 0.06, 54) * eased;
    ...
    hero.style.setProperty(
      "--hero-media-scale",
      (1.09 + eased * 0.18).toFixed(4),
    );
    hero.style.setProperty("--hero-copy-y", `${(-copyTravel).toFixed(2)}px`);
    hero.style.setProperty(
      "--hero-copy-opacity",
      Math.max(0, 1 - eased * 1.18).toFixed(3),
    );
  };
