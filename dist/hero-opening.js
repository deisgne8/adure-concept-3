...
let done=false,visible=true,measuredViewport=null,openingStartedAt=0;
...
window.addEventListener('resize',()=>{
  ...
  if(!done&&performance.now()-openingStartedAt>500&&measuredViewport&&(Math.abs(innerWidth-measuredViewport.width)>2||Math.abs(innerHeight-measuredViewport.height)>2))finish();
});
...
async function open(){
  if(!root.dataset.opening){finish();return;}
